...
def match_features(img1_path, img2_path, bbox_path_1, bbox_path_2, show=True):
    image1 = load_images(img1_path)
    image2 = load_images(img2_path)

    print("Getting first image keypoints")
    keypoints_1, descriptors_1 = extract_keypoints(img1_path, bbox_path_1, show_keypoints=False, show_mask=False, save=False)
    print("Getting second image keypoints")
    keypoints_2, descriptors_2 = extract_keypoints(img2_path, bbox_path_2, show_keypoints=False, show_mask=False, save=False)



    print("Matching initial keypoints")
