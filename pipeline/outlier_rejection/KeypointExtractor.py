...
def extract_keypoints(image_path, bounded_box_path, output_directory_path, show_keypoints, show_mask, save=True, save_img_with_keypoints=False):
    img = load_image(image_path)
    image_name = os.path.basename(image_path)
    boxes = load_bounded_boxes(bounded_box_path)
    new_mask = np.zeros_like(img[:, :, 0])

    print("Image is undergoing masking")
    ...
    print("Image is undergoing SIFT")
